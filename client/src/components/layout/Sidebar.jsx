...
const Sidebar = ({ isOpen, onClose, collapsed, onToggleCollapse }) => {
  ...
  const sidebarContent = useMemo(() => (
    <>
      <div className="sticky top-0 z-10 flex h-16 shrink-0 items-center justify-between border-b border-gray-100 bg-white px-6 dark:border-gray-800 dark:bg-gray-900">
        <div className="flex items-center gap-2">
          <img src="/img/najah-circle-removebg-preview.png" alt="Najah" className="w-10 h-10 object-contain" />
          <span className="text-lg font-bold text-gray-900 dark:text-white">Najah</span>
        </div>
        <button
          onClick={onClose}
          type="button"
      ...
      <div className="shrink-0 border-b border-gray-100 p-4 dark:border-gray-800">
        <div className="flex items-center gap-3 rounded-2xl bg-gray-50 p-3 dark:bg-gray-800/40">
            ...
            {(() => {
              ...
              return (
                <div className="relative h-full w-full">
                  ...
                  <div className="flex h-full w-full items-center justify-center rounded-full bg-gradient-to-br from-[#FFB900] to-[#0084D1] text-sm font-bold text-white">
                    {initials}
                  </div>
                </div>
              );
          ...
          <div className="overflow-hidden">
            ...
            <p className="truncate text-[10px] text-gray-400 dark:text-gray-500" title={user?.department || 'N/A'}>
              {user?.department || 'N/A'}
            </p>
          </div>
        </div>
      ...
      <nav className="flex-1 space-y-1 overflow-y-auto p-4 no-scrollbar">
        {menuItems.map((item) => (
          <NavLink
            ...
            className={({ isActive }) =>
              isActive
                ? 'flex items-center gap-3 rounded-xl bg-amber-400 px-4 py-3 text-sm font-semibold text-sky-600 transition-all dark:bg-amber-500/20 dark:text-sky-400 min-h-[44px]'
                : 'flex items-center gap-3 rounded-xl px-4 py-3 text-sm font-medium text-gray-500 hover:bg-gray-50 hover:text-gray-900 transition-all dark:text-gray-400 dark:hover:bg-gray-800 dark:hover:text-gray-200 min-h-[44px]'
            }
          >
      ...
      <div className="mt-auto shrink-0 border-t border-gray-200 p-4 dark:border-gray-700">
        <button
  ...
  ), [user, menuItems, t, onClose, handleLogout]);

  return (
    <>
      {/* Mobile sidebar via portal into document.body */}
      ...
      <aside dir="ltr"
        className={`hidden md:flex flex-col h-screen overflow-hidden border-r border-gray-200 bg-white transition-all duration-300 dark:border-gray-800 dark:bg-gray-900 sticky top-0 ${
          collapsed ? 'w-16' : 'w-64'
        }`}
      >
